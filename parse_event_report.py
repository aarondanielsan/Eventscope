...
POST_AS_RE = re.compile(r"Post As[:\-]?\s*(.+)", re.I)
...
def expand_grouped_space(line_text: str, primary_space: str | None) -> str | None:
    ...
    # Special case: Attache + Charge combined
    if re.search(r"\bAttache\b", line_text, re.I) and re.search(r"\bCharge\b", line_text, re.I):
...
def parse_pdf_to_csv(pdf_path):
    rows = []
    current_company, current_event, current_date, current_status = None, None, None, None
    last_space_by_event = {}
        ...
        for page in pdf.pages:
            ...
            for line in text.split("\n"):
                ...
                if "Quote #:" in l:
                    ...
                    in_event_header = True
                    in_event_body = False
                    continue
                if "Status:" in l:
                    status_match = re.search(r"Status:\s*(\w+)", l)
                    if status_match:
                ...
                # Post As lines
                m = POST_AS_RE.search(l)
                if m and rows and in_event_body:
                    post_as_val = m.group(1).strip()
                    last = rows[-1]
                    if last[7] and not last[-2]:
                        last[-2] = post_as_val
                    continue
                if m and in_event_header:
                    continue
                ...
                rows.append([
                    current_company or "", current_event or "", current_status or "", current_date or "",
                    start_time, end_time,
                    function_type or "", function_space or "", setup_style or "",
                    "",  # Post As placeholder
                    l
                ])
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "Company Name", "Event Name", "Event Status", "Day of Event",
            "Start Time", "End Time",
            "Function Type", "Function Space", "Setup Style", "Post As", "Raw Line"
        ])
